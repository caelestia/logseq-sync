...
- $$\operatorname{Hom}_G(P_p,k) \otimes_k \operatorname{Hom}_G(P_q,k) \xrightarrow{(-1)^{pq}\mu_{P_p,P_q}} \operatorname{Hom}_G(P_p\otimes P_q,k) \xrightarrow{\Delta_{p+q}^*} \operatorname{Hom}_G(P_{p+q},k)$$
	...
	- The Koszul sign $(-1)^{pq}$ is added, so that the above defines a map "about" chain complexes:
	- $$\operatorname{Hom}_{G}(P_\bullet,k) \otimes_k \operatorname{Hom}_{G}(P_\bullet,k) \xrightarrow{\} \operatorname{Hom}_G(P_p\otimes P_q,k) \xrightarrow{\Delta_{p+q}^*} \operatorname{Hom}_G(P_{p+q},k)$$

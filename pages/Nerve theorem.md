...
- For each $q\geq0$, we have the Cech complex of the singular $q$-chains functor. Observe that this complex is acyclic. Indeed, let $\sigma$ be a singular $q$-simplex, $I_\sigma$ be $\{i:\operatorname{im}(\sigma)\subset U_i\}$, then the subcomplex generated by $\sigma$ is
  $$\bigoplus_{\alpha\in I^2}S_q(U_\alpha)\longrightarrow\bigoplus_{\alpha\in}$$
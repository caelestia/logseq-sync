...
- Closed submanifold $Y\subset X$ of codimension $k$. If around each $x$ there is an open nhood $U$ and a submersion $f:U\to\C^k$ such that $Y\cap U=f^{-1}(0)$.
	- Example. Complete intersection in $\mathbb{P}^n$. If $k\leq n$, $(F_i)_{i=1}^k$ homogeneous polynomials in $n+1$ variables, satisfy $\det\left(\frac{\partial F_i}{\partial z_j}\right)_{i,j}$ has rank $k$ on $\{x\in\C^{n+1}:F_1=\cdots=F_k=0\}$, then the zero locus
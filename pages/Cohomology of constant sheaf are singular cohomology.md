...
- For injectivity, suppose $[x]$, $x\in S(X)$, is a class in $H^i(X,S^\bullet)$ which maps to $0$ in $H^i(X,\tilde S^\bullet)$. In other words, $x\mapsto dt$ for some $t\in\tilde S^{i-1}(X)$. Lift $t$ to $s\in S^{i-1}(X)$, then again $x-ds$
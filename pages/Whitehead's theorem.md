...
- Corollary. If $f:X\to Y$ is a weak homotopy equivalence and $X,Y$ are homotopy equialent to CW complexes, then $f$ is a homotopy equivalence.
	- Proof. Let $\psi:Y'\to Y$ be a homotopy equivalence where $Y'$ is a CW complex. By the proposition, there is $\phi:Y'\to X$ such that $[f\phi]=[\psi]$. Thus $f$ has a right homotopy inverse $g$. By the same logic, $g$ has a right homotopy inverse $
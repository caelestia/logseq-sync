...
- Proof. $\iota_0$ is a natural transformation, and so is $(\iota_0)_*=S_\bullet\circ\iota_0:S_\bullet\to T_\bullet$, where $T_\bullet(-):=S_\bullet(-\times I)$. On $\mathscr{C}=\mathsf{Top}$, we choose $\mathcal{M}=\{\Delta^p\}_{p\geq0}$. Then $S_n$ is free on the basis $\{\mathrm{id}_{\Delta^n}\}$.
- It is clear that $H_0(\iota_0)=H_0(\iota_1)$.
- To apply the uniqueness, it remains to check that $T$ is acyclic.
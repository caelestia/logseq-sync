...
- Case 3, $M\sim I_{1,1}$. This is the non-trivial case?
	- Case a) Every $\rho(\gamma)$ is diagonalizable. Then they are simultaneously diagonalizable.
		- If the eigenvalues of $\rho(\gamma)$ are $\{a,b\}$, then $\{a^{-1},b^{-1}\}=\{a,b\}$, so we have either:
		...
		- Case ii) There is at least one $\rho(\gamma)$ which has eigenvalues $\lambda \neq \lambda^{-1}$.
			...
			- Thus $a=d=0$. By $M^2=1$ we get $bc=1$. So $M=\begin{pmatrix}0&b\\b^{-1}&0\end{pmatrix}$.
	- Case b) There is some $\lambda$ s.t. $\rho(\lambda)$ is not diagonalizable.
		...
		- Let $M=\begin{pmatrix}a&b\\c&d\end{pmatrix}$ and solve $RM=MR^{-1}$ and $M^2=1$.
			...
			- In fact, there's only two cases up to gauge symmetry: $M=\begin{pmatrix}a&0\\0&-a\end{pmatrix}$.

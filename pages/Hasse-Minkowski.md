...
- Computation of the Hilbert symbol.
	...
	- For $v=p$, let $a=p^i\alpha$ and $b=p^j\beta$ where $\alpha,\beta\in\Z_p^\times$. Put $r:=(-1)^{ij}a^jb^{-i}\in\Z_p^\times$. Then:
		...
		- Proof. Check that RHS satisfies $(a,b)=(b,a)$, $(\lambda^2,b)=1$, $(a,bc)=(a,b)(a,c)$ and $(a,-a)=1$ directly. Thus reduce to the case $a\in\Z_p^\times$ and $b\in\Z_p^\times\cup p\Z_p^\times$. The rest is
-
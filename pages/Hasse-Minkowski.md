...
- Lemma. Let $k$ be a field, $a,b\in k$. Then $c\in k^\times$ is a norm of $k[\sqrt{a},\sqrt{b}]/k[\sqrt{ab}]$ iff it's the product of a norm of $k[\sqrt{a}]/k$ with a norm of $k[\sqrt{b}]/k$.
	...
	- Suppose that $x\in k[\sqrt{a}]$, $y\in k[\sqrt{b}]$. Then $x\tau x\cdot y\sigma y=(xy)\sigma\tau (xy)$, so the "if" is trivial. For the "only if", let $z\in k[\sqrt{a},\sqrt{b}]$ satisfy $z\cdot\sigma\tau z=c$.